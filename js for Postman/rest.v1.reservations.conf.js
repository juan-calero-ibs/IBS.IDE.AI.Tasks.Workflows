// Postman Test + Visualizer Script
// Includes: reservationId extraction, supplier fields, dates w/ ISO offset respect,
// fallback Check-in/Check-out from beginDate + duration (with source indicators),
// segments/products/parties/auths/udf tables, and Get Product/Get Price fetch panels.
...
(function () {
  ...
  if (pm.response.code === 200) {
    ...
    // ✅ NEW: beginDate + duration fallback for check-in/out (with provenance)
    function normalizeIsoOffset(iso) {
      return (iso && typeof iso === 'string')
        ? iso.replace(/([+-]\d{2})(\d{2})$/, '$1:$2')
        : iso;
    }
    ...
    function tryParseIso(iso) {
      ...
      const d = new Date(normalizeIsoOffset(iso));
      return isNaN(d.getTime()) ? null : d;
    }

    function fmtPrettyFromDateUTC(d) {
      if (!d || isNaN(d.getTime())) return { dateStr: "N/A", timeStr: "" };
      const parts = new Intl.DateTimeFormat('en-US', {
        timeZone: 'UTC',
        weekday: 'long',
        month: 'long',
      ...
      const map = Object.fromEntries(parts.map(p => [p.type, p.value]));
      return {
        dateStr: `${map.weekday}, ${map.month} ${map.day} ${map.year}`,
    ...
    function computedDateLine({ baseIso, computedDate, label }) {
      const utc = getUTCOffset(baseIso);
      const { dateStr, timeStr } = fmtPrettyFromDateUTC(computedDate);
      return `🧮 COMPUTED (${label}) ❗ ${dateStr} ❗ ${timeStr} ${utc}`;
    }

    function deriveStayDates(entity) {
      const checkoutIsoRaw =
        entity?.checkoutDate ||
      ...
      let checkinLine = "N/A";
      if (entity?.checkinDate) {
        checkinLine = fullDateLine(entity.checkinDate);
      } else if (entity?.beginDate) {
        checkinLine = `${fullDateLine(entity.beginDate)} 🧩 source: beginDate`;
      }

      let checkoutLine = "N/A";
      if (checkoutIsoRaw) {
        checkoutLine = fullDateLine(checkoutIsoRaw);
      } else if (entity?.beginDate && Number.isFinite(Number(duration))) {
        const baseD = tryParseIso(entity.beginDate);
        const outD = addDays(baseD, Number(duration));
        if (outD) {
          checkoutLine = computedDateLine({
            baseIso: entity.beginDate,
            computedDate: outD,
            label: `beginDate + duration (${duration})`
          });
        }
      }
    ...
    const stay = deriveStayDates(reservation);
    const checkinLine  = stay.checkinLine;
    const checkoutLine = stay.checkoutLine;
    ...
    const segmentRows = segments.map((s, idx) => {
      ...
      // ✅ NEW: stay date fallback for segments too
      const segStay = deriveStayDates(s);
      const segCheckin  = segStay.checkinLine;
      const segCheckout = segStay.checkoutLine;
    ...
    pm.visualizer.set(`
      ...
      (function () {
        ...
        async function runFetch(opts){
          ...
          const id    = opts.id;
          const rowIdx= opts.rowIdx;
          const path  = opts.path;
          const detId = opts.detId;
          const outId = opts.outId;
          ...
          const url = (CFG.protocol || 'https') + '://' + CFG.host + ':' + CFG.port + path + '/' + id;
          det.style.display = '';
          det.open = true;
